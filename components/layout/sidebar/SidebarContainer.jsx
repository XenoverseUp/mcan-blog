...
const SidebarContainer = ({ children }) => {
  ...
  return (
    <header
      ...
      className="relative hidden w-full flex-shrink-0 resize-x flex-col justify-between border-r border-border @container md:flex md:max-w-[468px] lg:max-w-[568px] min-[1240px]:max-w-[764px]"
    >
      <span
        draggable
        onDragStart={start}
      ...
      />
      <span className="pointer-events-none absolute right-0 top-1/2 z-30 h-6 w-4 -translate-y-1/2 translate-x-1/2 rounded-full border border-border bg-white before:absolute before:left-1/2 before:top-2 before:h-[1.5px] before:w-1/2 before:-translate-x-1/2 before:rounded-full before:bg-orange-500 after:absolute after:bottom-2 after:left-1/2 after:h-[1.5px] after:w-1/2 after:-translate-x-1/2 after:rounded-full after:bg-orange-500"></span>
      {children}
    </header>
  )
}
