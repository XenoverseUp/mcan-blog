...
export const CreatePostSchema = object({
  ...
  subtitle: union([
    z.string().min(6, {
      message:
        "`subtitle` must be either left empty or have least 6 characters.",
    }),
    z.string().length(0),
  ])
    .optional()
  ...
  coverImage: union([
    string().url({ message: "`coverImage` must be a url." }),
    string().length(0),
  ])
    .optional()
    .transform(e => (e === "" ? undefined : e)),
  ...
  previewUrl: union([
    string().url({ message: "`previewUrl` must be a url." }),
    string().length(0),
  ])
    .optional()
    .transform(e => (e === "" ? undefined : e)),
  repositoryUrl: union([
    string().url({ message: "`repositoryUrl` must be a url." }),
    string().length(0),
  ])
    .optional()
    .transform(e => (e === "" ? undefined : e)),
